...
var registroPost = function(req, res, next)
{
	...
	db.queryMysql(sql, function(err, response)
	{
		...
		else
		{
            ...
            sql = `inseert into ('nombre', 'usuario', 'clave', 'email','fecha') values ('${data.nombre}', '${data.username}', '${password}', '${data.correo}','${fechaActual}')`;//VARIABLE QUE GUARDARÁ LA SENTENCIA SQL...
			db.queryMysql(sql, function(err, response)
			{
