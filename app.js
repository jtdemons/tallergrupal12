...
var express 		= 	require("express"),
	...
	passport.use(new LocalStrategy(function(username, password, done)
	{
		...
		var sql = `SELECT * FROM users WHERE usuario = '${username}'`; 
		db.queryMysql(sql, function(err, response)
		{
			...
			*/
			...
			let clave = bcrypt.compareSync(password, response[0].clave);
	...
	passport.deserializeUser(function(username, done)
	{
		...
		var sql = `SELECT nombre, email, idusuario FROM users WHERE usuario = '${username}'`;//VARIABLE QUE GUARDARÁ LA SENTENCIA SQL...
		db.queryMysql(sql, function(err, response)
		{
			if(response)
			{
